...
long mode (long *arr, int n) {
  ...
  for(int i = 1 ; i < n ; i++)
  {
    if(arr[i] == numb)      //compares the first element to everything in array
    {
      count++;
    }
    else
    {
      if(count >  mod_counter)        //if the counter >= 1 the mode is currently the first element
      {
        mod_counter = count;
        mode = numb;
      }

      count = 0;                    // no mode found , reset eveything and compare element i to the rest of the array
      numb = arr[i];
    }
  }
  return mode;
